# -*- coding: utf-8 -*-
"""76 EWMA增强_ExtraTrees —— EWMA 分数增强变体。
...
与基模型 09_ExtraTrees 的唯一差别: 分数经逐机组因果 EWMA(span=12, 即2小时) 平滑。
...
t0 = now()
m = ExtraTreesClassifier(n_estimators=200, min_samples_leaf=5,
                         class_weight="balanced_subsample", n_jobs=-1,
                         random_state=0).fit(Xtr, ytr)

sva = ewma(m.predict_proba(Xva)[:, 1], TB_VAL, span=12)
ste = ewma(m.predict_proba(Xte)[:, 1], TB_TEST, span=12)
report("76_EWMA增强_ExtraTrees", yva, sva, yte, ste, now() - t0,
       extra={"增强": "EWMA(span=12, 逐机组因果)", "基模型": "09_ExtraTrees",
