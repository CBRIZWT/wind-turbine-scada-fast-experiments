...
与基模型 38_软投票集成 的唯一差别: 分数经逐机组因果 EWMA(span=12, 即2小时) 平滑。
...
t0 = now()
m = VotingClassifier(voting="soft", estimators=[
    ("lgbm", lgb.LGBMClassifier(n_estimators=300, num_leaves=63, learning_rate=0.05,
                                class_weight="balanced", random_state=0, n_jobs=-1, verbosity=-1)),
...
report("72_EWMA增强_软投票集成", yva, sva, yte, ste, now() - t0,
       extra={"增强": "EWMA(span=12, 逐机组因果)", "基模型": "38_软投票集成",
