# -*- coding: utf-8 -*-
"""73 EWMA增强_LightGBM —— EWMA 分数增强变体。
...
与基模型 13_LightGBM 的唯一差别: 分数经逐机组因果 EWMA(span=12, 即2小时) 平滑。
...
t0 = now()
m = lgb.LGBMClassifier(n_estimators=600, num_leaves=63, learning_rate=0.05, subsample=0.8,
...
report("73_EWMA增强_LightGBM", yva, sva, yte, ste, now() - t0,
       extra={"增强": "EWMA(span=12, 逐机组因果)", "基模型": "13_LightGBM",
